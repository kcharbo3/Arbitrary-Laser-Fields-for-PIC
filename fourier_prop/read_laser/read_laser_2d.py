...
def compute_field_at_sim_grid(input_field: laser_input.InputField, comm, rank, num_processes, verbose=False):
    sim_grid_parameters = grid.compute_sim_grid(
        input_field.prop.times, input_field.prop.y_vals_output
    )
    ...
    Et_sim_y[y_start_half_sim:y_end_half_sim, :] = \
        interp_func((y_chunk_half, t_chunk))
    Et_sim_y.flush()
    del Et_sim_y
    del interp_func
    del t_chunk, y_chunk_half
...
def create_interpolation_functions(input_field: laser_input.InputField, sim_grid_parameters, comm, rank, num_processes):
    ...
    Ez_file = input_field.get_output_Et_field_file_z_2d()
    ...
    Et_file_y = input_field.get_output_Et_field_file_y_2d()
    Et_file_z = input_field.get_output_Et_field_file_z_2d()
    ...
    Et_chunk_z = Et_file_z[start_index:end_index, t_index_lo:t_index_hi + 1]
...
def create_Et_sim_files(input_field: laser_input.InputField, num_t_vals, num_y_vals):
    np.memmap(
        input_field.prop.data_directory_path + constants.OUTPUT_ET_SIM_FILE_Y, dtype='complex64',
        mode='w+', shape=(num_y_vals, num_t_vals)
    )
...
def get_Et_sim_file_y(input_field: laser_input.InputField, num_t_vals, num_y_vals):
    return np.memmap(
        input_field.prop.data_directory_path + constants.OUTPUT_ET_SIM_FILE_Y, dtype='complex64',
        mode='r+', shape=(num_y_vals, num_t_vals)
    )


def get_Et_sim_file_z(input_field: laser_input.InputField, num_t_vals, num_y_vals):
    return np.memmap(
        input_field.prop.data_directory_path + constants.OUTPUT_ET_SIM_FILE_Z, dtype='complex64',
        mode='r+', shape=(num_y_vals, num_t_vals)
    )
...
def _get_total_energy(E_mag, comm, rank, num_processes, dy, dt):
    total_sum = (E_mag ** 2).sum() * (dy * dt)
